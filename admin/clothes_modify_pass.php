...
$sql="update clothes set title=?,comefrom=?,pubdate=?,thumbnail=?,keywords=?,description=?,content=?,posid=? where id=?";
$sth = $dbh->prepare($sql);
$sth->execute(array($_POST['title'],$_POST['comefrom'],$pubdate,$_POST['thumbnail'],$keywords,$description,$_POST['content'],$posid,$_GET['id']));
echo "<script>alert('修改成功！');window.location.href='clothes_list.php';</script>";
$dbh = null;
?>
</body>
</html>

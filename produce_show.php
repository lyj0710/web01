    ...
    <div class="cont">
      ...
      <article>
        ...
        <div class="pinglun" style="width:100%;">
            ...
            <h4>请写下你的评论</h4>
            <form class="content" action="pro_comment_add.php?id=<?=$row['id']?>" method="post">
                <div class="cont-box">
        ...
        <div id="info-show">
            ...
            <?php 
                //记录的总条数
                    $total_num=$dbh->query("select id_art from pro_comments where id_art ='".$_GET['id']."' ")->rowCount();
                //设置每页显示的记录数
                    $pagesize=6;
                //计算总页数
                ...
                //设置页数
                    ...
                    //读取指定的记录数
                    $sql="select * from pro_comments where id_art ='".$_GET['id']."' order by id desc limit $offset,$pagesize";
                    $rows = $dbh->query($sql);
                        if($total_num>0){
                            foreach($rows as $row){
                    ?>
